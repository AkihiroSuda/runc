...
struct nlconfig_t {
	...
	uint8_t launched_with_non_zero_euid;
	char *uidmappath;
...
#define LAUNCHED_WITH_NON_ZERO_EUID_ATTR	    27287
...
static void nl_parse(int fd, struct nlconfig_t *config)
{
	...
	while (current < data + size) {
		...
		case CLONE_FLAGS_ATTR:
			...
			break;
		case LAUNCHED_WITH_NON_ZERO_EUID_ATTR:
			config->launched_with_non_zero_euid = readint8(current);
			break;
	...
	case JUMP_PARENT:{
			...
			while (!ready) {
				...
				case SYNC_USERMAP_PLS:
					...
					if (config.launched_with_non_zero_euid && !config.is_setgroup)
