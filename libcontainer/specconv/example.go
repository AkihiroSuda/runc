...
func ToRootlessWithContext(ctx RootlessContext, spec *specs.Spec, opts *RootlessOpts) error {
	...
	if ctx.InUserNS {
		uNextContainerID := 0
		sort.Slice(ctx.UIDMap, func(i int, j int) bool { return ctx.UIDMap[i].ID < ctx.UIDMap[i].ID })
		...
		gNextContainerID := 0
		sort.Slice(ctx.GIDMap, func(i int, j int) bool { return ctx.GIDMap[i].ID < ctx.GIDMap[i].ID })
		...
		if opts.MapSubUIDGID {
			uNextContainerID := 1
			sort.Slice(ctx.SubUIDs, func(i int, j int) bool { return ctx.SubUIDs[i].SubID < ctx.SubUIDs[i].SubID })
			...
			gNextContainerID := 1
			sort.Slice(ctx.SubGIDs, func(i int, j int) bool { return ctx.SubGIDs[i].SubID < ctx.SubGIDs[i].SubID })
