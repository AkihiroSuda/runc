...
func loadFactory(context *cli.Context) (libcontainer.Factory, error) {
	...
	lenient, err := shouldUseLenientCgroupManager(context)
	if err != nil {
		return nil, err
	}
	if lenient {
		cgroupManager = libcontainer.LenientCgroupfs
	}
...
func createContainer(context *cli.Context, id string, spec *specs.Spec) (libcontainer.Container, error) {
	lenientCgroup, err := shouldUseLenientCgroupManager(context)
	if err != nil {
		return nil, err
	}
	config, err := specconv.CreateLibcontainerConfig(&specconv.CreateOpts{
		...
		Spec:             spec,
		LaunchedWithNonZeroEUID: os.Geteuid() != 0,
		LenientCgroup:           lenientCgroup,
	})
	if err != nil {
		return nil, err
	}
